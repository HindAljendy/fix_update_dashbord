...
export default function ExploreApiActiveties() {
    const [Explore, setExplore] = useState([]);
    const [chosingLang , setchosingLang] = useState('en')
    const [chosingExploreKind , setchosingExploreKind] = useState('Activeties')
    ...
    useEffect(() => {
      const fetchData2 = async () => {
        try {
          ...
          console.log(Explore[0].images[0].image);
        } catch (error) {
          console.error(error);
        }
      };
    ...
    const changeLanguage = (newLanguage) => {
      ...
      if(newLanguage === 'ar')
      {
        setchosingExploreKind('النشاطات')
      }else
      {
        setchosingExploreKind('Activeties')
      }
    };

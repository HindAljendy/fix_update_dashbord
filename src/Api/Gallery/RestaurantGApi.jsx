...
export default function RestaurantGAPI({start,end}) {

    const [RestaurantPHoto, setRestaurantPHoto] = useState([]);
    const [chosingPhotoKind, setchosingPhotoKind] = useState('Restaurant')
    ...
    useEffect(() => {
        const fetchData2 = async () => {
            try {
                ...
                const response = await axios.get(`http://127.0.0.1:8000/api/images?category=${chosingPhotoKind}`, config);
